...
export class AdminPredicates {
  ...
  /**
   * Configures a predicate with the given inputs. For the values, it uses the first populated parameter in this order:
   * 1. complexValues
   * 2. values
   * 3. value
   */
  async configurePredicate({
    ...
    const valuesToSet = complexValues
      ? complexValues
      : values
  ...
  async fillValue(
    ...
    if (valueInput) {
      ...
      const secondValueInput = await this.page.$(
        `input[name="group-${groupNum}-question-${questionId}-predicateSecondValue"]`,
      )
      if (secondValueInput) {
